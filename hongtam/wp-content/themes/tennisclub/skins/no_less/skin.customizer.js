...
function tennisclub_skin_customizer(option, val) {
"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = TENNISCLUB_GLOBALS['co_add_params']!=undefined ? TENNISCLUB_GLOBALS['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#tennisclub-customizer-styles-'+option).length > 0 ? jQuery('#tennisclub-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="tennisclub-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
