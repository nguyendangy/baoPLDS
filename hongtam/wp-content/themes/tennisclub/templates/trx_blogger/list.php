...
if ( !function_exists( 'tennisclub_template_list_theme_setup' ) ) {
	add_action( 'tennisclub_action_before_init_theme', 'tennisclub_template_list_theme_setup', 1 );
	function tennisclub_template_list_theme_setup() {
		tennisclub_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'tennisclub')
			));
	}
}

// Template output
if ( !function_exists( 'tennisclub_template_list_output' ) ) {
	function tennisclub_template_list_output($post_options, $post_data) {
