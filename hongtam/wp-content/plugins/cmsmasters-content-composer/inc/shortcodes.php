...
public function cmsmasters_icon_box($atts, $content = null) {
	...
	$out .= '<div id="cmsmasters_icon_box_' . $unique_id . '" class="cmsmasters_icon_box ' . $box_type . ' box_icon_type_' . $box_icon_type . 
...
public function cmsmasters_image($atts, $content = null) {
	...
	if ($content != null) {
		...
		if (is_numeric($new_image_thumb[0])) {
			...
			$new_image_alt = trim(strip_tags(get_post_meta($new_image_thumb[0], '_wp_attachment_image_alt', true)));
...
public function cmsmasters_social($atts, $content = null) {
	...
	if ($facebook == 'true' || $twitter == 'true' || $pinterest == 'true') {
		...
		if ($facebook == 'true') {
			$out .= '<div class="share_wrap">' . "\n" . 
				'<a href="//www.facebook.com/sharer/sharer.php?display=popup&u=' . $page_link . '" class="button cmsmasters-icon-facebook">' . esc_html__('Facebook', 'cmsmasters_content_composer') . '</a>' . "\n" . 
			'</div>' . "\n";
		}
...
";
	...
	$orderby = ($orderby == 'popular') ? 'meta_value_num title' : $orderby;
	...
	if ($orderby == 'meta_value_num title') {
	...
	if ($query->have_posts()) : 
		while ($query->have_posts()) : $query->the_post();
			if ($layout == 'columns') {
				if (get_post_format() != '') {
					$out .= cmsmasters_composer_ob_load_template('framework/post-type/blog/page/masonry/' . get_post_format() . '.php', $this->blog_atts);
				} else {
					$out .= cmsmasters_composer_ob_load_template('framework/post-type/blog/page/masonry/standard.php', $this->blog_atts);
				}
			} elseif ($layout == 'timeline') {
				if (get_post_format() != '') {
					$out .= cmsmasters_composer_ob_load_template('framework/post-type/blog/page/timeline/' . get_post_format() . '.php', $this->blog_atts);
				} else {
					$out .= cmsmasters_composer_ob_load_template('framework/post-type/blog/page/timeline/standard.php', $this->blog_atts);
				}
			} else {
				if (get_post_format() != '') {
					$out .= cmsmasters_composer_ob_load_template('framework/post-type/blog/page/default/' . get_post_format() . '.php', $this->blog_atts);
				} else {
					$out .= cmsmasters_composer_ob_load_template('framework/post-type/blog/page/default/standard.php', $this->blog_atts);
				}
			}
		endwhile;
...
public function cmsmasters_paypal_donations($atts, $content = null) {
	...
	if ( $options ) {
		...
		$out .= '<div class="cmsmasters_notice cmsmasters_notice_info">' . 
			'<a href="#" class="notice_close cmsmasters_theme_icon_cancel"></a>' .
			'<div class="notice_content"><p>' . esc_html__( 'Please enter a URL to a custom donation button in Settings – PayPal Donations tab.', 'cmsmasters_content_composer' ) . '</p></div>' .
		'</div>';
	}
