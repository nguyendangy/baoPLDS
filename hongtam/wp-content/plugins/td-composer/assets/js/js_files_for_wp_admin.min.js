...
(a.stopPropagation(),a.preventDefault(),d=tdcWindowFrame.getMouseInitialPosition(),"undefined"!==typeof d)){var k={offsetLeft:0,offsetTop:0};_.isUndefined(tdcWindowFrame._resizeOffset)||(k=tdcWindowFrame._resizeOffset);c=tdcWindowFrame.getCurrentWindowFramePosition();d=a.pageX-c.left+k.width-k.offsetLeft;a=a.pageY-c.top+k.height-k.offsetTop;tdcWindowFrame.$_currentWindowFrame.css({width:d,height:a});tdcWindowFrame.$_currentWindowFrame.data("width",d);tdcWindowFrame.$_currentWindowFrame.data("height",
...
b('<div class="tdc-panel-button"><div class="tdc-panel-icon"></div><div class="tdc-panel-link"><a onclick="return tdcInit.savePost( event )" href="'+tdcInit._url+'">TagDiv Composer</a></div></div>').insertAfter("div#titlediv")}},_checkPostContent:function(){tdcInit.$title.length&&tdcInit.$titlePromptText.length&&0===tdcInit.$title.val().trim().length&&(tdcInit.$title.val(tdcInit._title),tdcInit.$titlePromptText.hide());tdcShortcodeParser.init({0:["vc_row"],1:["vc_column"],2:["vc_row_inner"],3:["vc_column_inner"],
4:[]});return tdcInit.$contentWrap.hasClass("tmce-active")?tinyMCE.activeEditor.getContent():tdcInit.$formPost.find("#content").val()},savePost:function(a){b(".tdc-panel-button").addClass("tdc-open-editor-loader");setTimeout(function(){b(".tdc-panel-button").removeClass("tdc-open-editor-loader")},2E3);a=tdcInit._checkPostContent();b.ajax({url:window.ajaxurl,type:"POST",data:{"data[wp_autosave][post_id]":tdcInit.$postId.val(),"data[wp_autosave][post_type]":tdcInit.$postType.val(),"data[wp_autosave][post_author]":tdcInit.$postAuthor.val(),
"data[wp_autosave][post_title]":tdcInit.$title.val(),"data[wp_autosave][content]":a,"data[wp_autosave][excerpt]":tdcInit.$excerpt.val(),"data[wp_autosave][catslist]":tdcInit.catslistValue,"data[wp_autosave][auto_draft]":1,"data[wp_autosave][_wpnonce]":tdcInit.$_wpnonce.val(),interval:60,_nonce:window.heartbeatSettings.nonce,action:"heartbeat",screen_id:tdcInit.$postType.val(),has_focus:!1}}).done(function(a,b,d){window.location=tdcInit._url});return!1}};b(window).ready(function(){tdcInit.init()})})(jQuery,
_);var tdcDebug;
